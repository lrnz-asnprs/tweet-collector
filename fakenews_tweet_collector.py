...
#sys.path.append("/fake_collector/configs")
...
politifact = pd.read_csv(str(dir.DATA_PATH)+"/fakenews_sources/politifact_scrape_7t_03102022.csv")
...
sample.to_csv("sample_tweets_for_run0310.csv")
...
def reshape_date_format(sample):
    sample['stated_on'] = sample.stated_on.apply(lambda x: str(datetime.strptime(x, '%B %d, %Y').isoformat()+"Z"))
...
path = str(dir.DATA_PATH)+"/fakenews_tw_output/"
